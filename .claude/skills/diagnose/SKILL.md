...
Приоритет роста: [слот на языке диалога]
...
Bottleneck labels (RU / EN — на языке диалога; сохраняется русский термин в скобках при EN):
- cp.rhy → «регулярность и ритм занятий» / "consistency and rhythm (регулярность и ритм занятий)"
- cp.wld → «мировоззрение и системный взгляд» / "worldview and systems thinking (мировоззрение и системный взгляд)"
- cp.skl → «учёт времени и собранность» / "time tracking and focus (учёт времени и собранность)"
- cp.iwe → «рабочая среда и инструменты» / "work environment and tools (рабочая среда и инструменты)"
- cp.int → «системное мышление» / "systems thinking (системное мышление)"
- cp.agt → «агентность и инициатива» / "agency and initiative (агентность и инициатива)"
- 'none' → «Узких мест нет — поддерживайте темп и берите следующие ступени» / "No bottlenecks — keep the pace and take the next stages"
...
[Приоритет роста: <bottleneck на языке диалога>]
ИЛИ
...
```
Следующий шаг:
→ Навигатор, как развивать [bottleneck на языке диалога]? — рекомендации по росту
