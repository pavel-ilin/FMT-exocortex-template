...
- **Язык вывода.** Внутренний контракт вердикта (`GO`/`CAUTION`/`BLOCKED`,
  ...
  **Одну итоговую строку решения пилоту в чат отдавать на языке диалога** (канал-детектор
  DP.SC.050; RU): «Публиковать безопасно / Публиковать нельзя — <причина>»; (EN): "Safe to publish / Do not publish — <reason>".
...
После вердикта — **одна строка пилоту в чат на языке диалога**: (RU) «Публиковать безопасно (<тег>)» / «Публиковать нельзя — <главная причина>» / «Не удалось проверить — <чего не хватает>»; (EN) "Safe to publish (<tag>)" / "Do not publish — <main reason>" / "Could not verify — <what is missing>".
