...
> **Стиль текста:** ретро недели и новый WeekPlan читает пилот → весь текст синтезировать в базе разговорного стиля (S0 база + S1 автор, источник DP.SC.050) на языке диалога/артефакта (`lang:` WeekPlan), без машинных меток, главная мысль первой, код РП и путь не подлежащее.
...
- [ ] Peer-сессии недели: WP Gate проверен (только сессии с 2026-06-09):
  ...
  find ~/IWE/${IWE_GOVERNANCE_REPO:-DS-strategy}/sessions -type f -name "peer-prompt.md" \
    ...
    | xargs -I{} sh -c 'grep -qE "Открытие \(WP Gate\)|Opening \(WP Gate\)" "{}" || echo "WP-GATE-MISS: {}"'
