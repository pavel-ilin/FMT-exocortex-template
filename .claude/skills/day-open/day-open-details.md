...
**Свип делает скрипт, не агент (issue #207):** секцию «Новые задачи в репозиториях» наполняет `day-open-scaffold.sh` (`render_repo_issues`) — запускать каноническую копию `bash $IWE_SCRIPTS/day-open-scaffold.sh` (WP-002 Ф3). Скрипт сам пропускает каталоги без `.git` и без GitHub-remote и кэширует ответ на 1ч. Вручную перебирать репо через `gh` не нужно — это удваивает токены на пустых репо.
⚠️ Не вызывать корневую `scripts/day-open-scaffold.sh` workspace: если рядом нет `lib/`, скрипт падает FATAL (issue #455). Свип GitHub-only: инсталляции на GitLab без `gh` честно деградируют — «gh CLI недоступен», не ошибка.
...
> Отвечай на языке установки (params.yaml `language`, default ru; для en — переводи смысл, названия собственных не трогай). Без перечисления всех новостей — только синтез.
> Входные данные:
